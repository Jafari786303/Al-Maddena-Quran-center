...
function setupRoleUI() {
  ...
  if (currentProfile.role === "student") {
    ...
    studentEnrollmentsCard.classList.remove("hidden");
    ...
    onSnapshot(q, (snap) => {
      ...
      snap.forEach((docSnap) => {
        ...
        row.innerHTML = `<strong>${e.mainCourse || e.courseId || ""}</strong><p>Tajweed Stage: ${e.tajweedStage || "N/A"}</p><p>Status: ${status}</p>`;
        studentEnrollments.appendChild(row);
        if (!firstAssigned && e.assignedTeacherId) firstAssigned = e;
      });

      if (firstAssigned?.assignedTeacherId) {
        currentChatId = [currentUser.uid, firstAssigned.assignedTeacherId].sort().join("__");
        chatWrap.classList.remove("hidden");
        chatMeta.textContent = `Teacher: ${firstAssigned.assignedTeacherName || "Assigned Teacher"}`;
