...
const langEn = $("#langEn");
const langUr = $("#langUr");
const SMARTFORM_ENDPOINT = "https://smartforms.dev/submit/6a02061ec184545ccc2286e1";

const COURSE_TREE = {
  intro_holy_quran: {
    label: "قرآنِ مجید کا تعارف (قرآنیات)",
    subs: [
      "Tajweed",
...
courseApplyForm.addEventListener("submit", async (e) => {
  ...
  await sendToSmartForm({
    form_type: "enrollment",
    uid: currentUser.uid,
    student_name: currentProfile.name || "",
    student_email: currentProfile.email || "",
    student_age: currentProfile.age || "",
    student_phone: currentProfile.phone || "",
    main_course: mainCourse,
    sub_category: subCategory,
    tajweed_stage: subCategory === "Tajweed" ? tajweedStage : "",
    duration_months: Number(fd.get("durationMonths") || 0),
    duration_days: Number(fd.get("durationDays") || 0),
    free_times: fd.get("freeTimes") || "",
    previous_experience: fd.get("previousExperience") || ""
  });
  alert("درخواست کامیابی سے جمع ہو گئی۔");
...
function renderProfile() {
  ...
  profileCard.innerHTML = `<img src="${currentProfile.photoUrl || "https://placehold.co/120x120"}" alt="dp"/><div><h4>${currentProfile.name || "نام موجود نہیں"}</h4><p>${currentProfile.email || ""}</p><p>عمر: ${currentProfile.age || "-"}</p><p>${currentProfile.phone || "نمبر موجود نہیں"}</p><p class="role-badge">${currentProfile.role === "student" ? "طالب علم" : currentProfile.role === "teacher" ? "استاد" : "ایڈمن"}</p>${currentProfile.role === "student" ? `<p class="score-row">اسکور: تجوید ${g.tajweed || 0} | تلفظ ${g.pronunciation || 0} | حاضری ${g.attendance || 0}</p>` : ""}<button id="logoutBtn">لاگ آؤٹ</button></div>`;
...
function setupRoleUI() {
  ...
  if (role === "student" || role === "admin") {
    ...
    roleOverview.textContent = role === "admin"
      ...
      : "طالب علم ڈیش بورڈ: کورس کے لیے اپلائی کریں، درخواست کی حالت دیکھیں، اور اسائن شدہ استاد سے چیٹ کریں۔";
    const q = query(collection(db, "enrollments"), where("studentId", "==", currentUser.uid));
    onSnapshot(q, (snap) => {
      const threadMap = new Map();
      if (!snap.docs.length) {
        studentEnrollments.innerHTML = "<p class='note'>ابھی تک کوئی درخواست جمع نہیں ہوئی۔</p>";
        chatThreads.innerHTML = "<p class='note'>ابھی کوئی چیٹ موجود نہیں۔</p>";
        return;
      }
      ...
      snap.forEach((docSnap) => {
        ...
        const action = canChat ? `<button data-chat-student="${e.assignedTeacherId}" data-teacher-name="${e.assignedTeacherName || "اسائن شدہ استاد"}">چیٹ کھولیں</button>` : "";
        const row = document.createElement("div");
        row.className = "user-list-item";
        row.innerHTML = `<strong>${e.mainCourse || e.courseId || ""}</strong><p>ذیلی زمرہ: ${e.subCategory || "لاگو نہیں"}</p><p>تجوید مرحلہ: ${e.tajweedStage || "لاگو نہیں"}</p><p>دورانیہ: ${e.durationMonths || 0} ماہ ${e.durationDays || 0} دن</p><p>حالت: ${status}</p>${action}`;
      ...
      studentEnrollments.querySelectorAll("button[data-chat-student]").forEach((btn) => {
        btn.onclick = () => {
          ...
          chatMeta.textContent = `استاد: ${teacherName}`;
  ...
  if (role === "teacher") {
    ...
    chatWrap.classList.remove("hidden");
    roleOverview.textContent = "استاد ڈیش بورڈ: اسائن شدہ طلبہ دیکھیں اور انفرادی چیٹس منیج کریں۔";
    ...
    onSnapshot(q, (snap) => {
      ...
      snap.forEach((d) => {
        ...
        div.innerHTML = `<strong>${e.studentName}</strong><p>${e.studentEmail}</p><button data-sid="${e.studentId}" data-name="${e.studentName}">چیٹ کھولیں</button>`;
      ...
      teacherStudents.querySelectorAll("button").forEach((b) => {
        b.onclick = () => {
          ...
          chatMeta.textContent = `طالب علم: ${b.dataset.name}`;
          subscribeToChat(currentChatId);
        };
      });
  ...
  if (role === "admin") {
    roleOverview.textContent = "ایڈمن ڈیش بورڈ: صارفین، درخواستیں اور استاد اسائنمنٹ منیج کریں۔";
  }
}
  ...
  threads.forEach((t) => {
    ...
    if (mode === "student") {
      ...
      btn.onclick = () => {
        ...
        chatWrap.classList.remove("hidden");
        chatMeta.textContent = `استاد: ${t.teacherName} | ${t.course}`;
        subscribeToChat(currentChatId);
      ...
      btn.onclick = () => {
        ...
        chatWrap.classList.remove("hidden");
        chatMeta.textContent = `طالب علم: ${t.studentName} | ${t.course}`;
        subscribeToChat(currentChatId);
        markThreadRead(currentChatId);
      };
    }
...
function setupCourseFormUX() {
  ...
  const refreshPdfPreview = () => {
    ...
    courseContentPreview.innerHTML = `<strong>${mainLabel}</strong><p>ذیلی زمرہ: ${sub || "-"}</p><a href="${url}" target="_blank" rel="noopener">پی ڈی ایف مواد کھولیں</a>`;
  };
...
async function loadAdminPanel() {
  ...
  usersSnap.forEach((d) => {
    ...
    const div = document.createElement("div");
    div.className = "user-list-item";
    div.innerHTML = `<strong>${u.name || "نام موجود نہیں"}</strong><p>ای میل: ${u.email || ""}</p><p>عمر: ${u.age || "-"}</p><p>فون: ${u.phone || "-"}</p><p>پتہ: ${u.address || "-"}</p><p>ذریعہ معلومات: ${u.heardFrom || "-"}</p><p>کردار: ${u.role || ""}</p><p>اکاؤنٹ آئی ڈی: ${u.uid || d.id}</p>`;
  ...
  enrollSnap.forEach((d) => {
    ...
    row.innerHTML = `<strong>${e.studentName}</strong><p>مرکزی کورس: ${e.mainCourse || e.courseId || ""}</p><p>ذیلی زمرہ: ${e.subCategory || "لاگو نہیں"}</p><p>تجوید مرحلہ: ${e.tajweedStage || "لاگو نہیں"}</p><p>دورانیہ: ${e.durationMonths || 0} ماہ ${e.durationDays || 0} دن</p><label>استاد منتخب کریں</label><select data-id="${d.id}"><option value="">غیر اسائن</option>${options}</select><button data-save="${d.id}">اس درخواست کے لیے استاد اسائن کریں</button>`;
...
async function sendToSmartForm(payload) {
  try {
    await fetch(SMARTFORM_ENDPOINT, {
      method: "POST",
