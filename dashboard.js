...
const i18n = {
  en: { dashboard: "Dashboard", quick: "Quick Actions", overview: "Overview" },
  ur: { dashboard: "ڈیش بورڈ", quick: "فوری اعمال", overview: "جائزہ" }
};
...
function renderThreadList(threads, mode) {
  ...
  threads.forEach((t) => {
    ...
    if (mode === "student") {
      btn.innerHTML = `<strong>${t.teacherName}</strong><span>${t.course}</span><span class="unread-dot hidden" data-unread-for="${t.teacherId}">0</span>`;
      btn.onclick = () => {
      ...
      btn.innerHTML = `<strong>${t.studentName}</strong><span>${t.course}</span><span class="unread-dot hidden" data-unread-for="${t.studentId}">0</span>`;
      btn.onclick = () => {
...
function setupCourseFormUX() {
  ...
  const fillSubCategories = () => {
    ...
    const subs = COURSE_TREE[main]?.subs || [];
  ...
  const refreshPdfPreview = () => {
    ...
    courseContentPreview.innerHTML = `<strong>${mainLabel}</strong><p>Subcategory: ${sub || "-"}</p>${url ? `<a href="${url}" target="_blank" rel="noopener">Open Hosted PDF Content</a>` : "<p class='note'>PDF not linked yet.</p>"}`;
  };
