...
courseCards.innerHTML = courses.map((c) => {
  const items = c.topics.map((t) => {
    const url = PDF_LINKS[t] || "https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf";
    return `<li><a href="${url}" target="_blank" rel="noopener">${t}</a></li>`;
  }).join("");
  const moreCount = c.title === "احکام" || c.title === "اخلاق" ? 24 : 23;
  return `<article class="course-card"><h4>${c.title}</h4><ul>${items}</ul><p class="note">+ ${moreCount} مزید موضوعات</p></article>`;
}).join("");
