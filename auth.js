...
const DEFAULT_PDF_LINK = "https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf";
...
function setupCourseMenus() {
  ...
  const groups = Object.entries(COURSE_TREE).slice(0, 4);
  let activeKey = groups[0]?.[0] || "";

  const renderSubMenus = () => {
    const activeCourse = COURSE_TREE[activeKey];
    const submenu = courseCards.querySelector("#courseSubMenu");
    if (!submenu || !activeCourse) return;

    const items = activeCourse.subs.map((topic) => {
      const url = PDF_LINKS[topic] || DEFAULT_PDF_LINK;
      return `<a class="submenu-link" href="${url}" target="_blank" rel="noopener">${topic}</a>`;
    }).join("");

    submenu.innerHTML = `
      <div class="course-panel-head">
        <div>
          <span class="eyebrow">Sub Menus</span>
          <h4>${activeCourse.label}</h4>
        </div>
        <span class="topic-count">${activeCourse.subs.length} topics</span>
      </div>
      <div class="submenu-grid">${items}</div>
    `;
  };

  courseCards.innerHTML = `
    <div class="course-menu" role="tablist" aria-label="Course menu headings">
      ${groups.map(([key, course], index) => `
        <button type="button" class="course-menu-btn${key === activeKey ? " active" : ""}" data-course-menu="${key}" role="tab" aria-selected="${key === activeKey}">
          <span>${String(index + 1).padStart(2, "0")}</span>
          <strong>${course.label}</strong>
        </button>
      `).join("")}
    </div>
    <article class="course-card course-panel" id="courseSubMenu"></article>
  `;

  courseCards.querySelectorAll("[data-course-menu]").forEach((button) => {
    button.addEventListener("click", () => {
      activeKey = button.dataset.courseMenu;
      courseCards.querySelectorAll("[data-course-menu]").forEach((btn) => {
        const isActive = btn.dataset.courseMenu === activeKey;
...
async function ensureProfile(user, fd) {
  ...
  await sendToSmartForm({
    ...
    heard_from: fd.get("heardFrom") || "",
    role,
    photo_url: fd.get("photoUrl") || user.photoURL || ""
  });
}
...
function setupSlider() {
  ...
  slidesTrack.style.direction = "ltr";
  slides.forEach((slide, index) => renderSlideMedia(slide, index));

  const render = () => {
    pauseInactiveVideos(slides, idx);
...
function renderSlideMedia(slide, index) {
  const existingMedia = slide.querySelector("img, video, iframe");
  if (existingMedia) return;

  const mediaUrl = String(slide.dataset.mediaUrl || "").trim();
  if (!mediaUrl) return;

  const caption = slide.querySelector(".slide-caption");
  const media = createSliderMedia(mediaUrl, index);
  slide.insertBefore(media, caption || null);
}

function createSliderMedia(url, index) {
  const embedUrl = getVideoEmbedUrl(url);
  if (embedUrl) {
    const iframe = document.createElement("iframe");
    iframe.src = embedUrl;
    iframe.title = `Slider video ${index + 1}`;
    iframe.loading = "lazy";
...
function pauseInactiveVideos(slides, activeIndex) {
...
async function sendToSmartForm(payload) {
  try {
    ...
    // Avoid breaking user flow if external endpoint fails.
