...
const subscribeDrawer = document.querySelector("#subscribeDrawer");
const openSubscribeBtn = document.querySelector("#openSubscribeBtn");
const closeSubscribeBtn = document.querySelector("#closeSubscribeBtn");
const confirmSubscribeBtn = document.querySelector("#confirmSubscribeBtn");
...
function setupSubscriptionUI() {
  openSubscribeBtn?.addEventListener("click", () => subscribeDrawer?.classList.remove("hidden"));
  closeSubscribeBtn?.addEventListener("click", () => subscribeDrawer?.classList.add("hidden"));
  confirmSubscribeBtn?.addEventListener("click", () => {
    const email = String(subscribeEmail?.value || "").trim();
    if (!email) return alert("براہ کرم ای میل درج کریں۔");
    localStorage.setItem("subscriber_email", email);
    alert("سبسکرپشن کامیاب! شکریہ۔");
    subscribeDrawer?.classList.add("hidden");
  });
}
